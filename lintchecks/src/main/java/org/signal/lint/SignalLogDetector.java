...
public final class SignalLogDetector extends Detector implements Detector.UastScanner {
  ...
  static final Issue LOG_NOT_SIGNAL = Issue.create("LogNotSignal",
                                                   "Logging call to Android Log instead of Pigeon's Logger",
                                                   "Pigeon has its own logger which must be used.",
                                                   Category.MESSAGES,
                                                   5,
                                                   Severity.ERROR,
  ...
  static final Issue LOG_NOT_APP = Issue.create("LogNotAppSignal",
                                                "Logging call to Pigeon Service Log instead of App level Logger",
                                                "Pigeon app layer has its own logger which must be used.",
                                                Category.MESSAGES,
                                                5,
                                                Severity.ERROR,
                                                new Implementation(SignalLogDetector.class, Scope.JAVA_FILE_SCOPE));
  ...
  public void visitMethodCall(JavaContext context, @NotNull UCallExpression call, @NotNull PsiMethod method) {
    ...
    if (evaluator.isMemberInClass(method, "android.util.Log")) {
      ...
      context.report(LOG_NOT_SIGNAL, call, context.getLocation(call), "Using 'android.util.Log' instead of a Pigeon Logger", fix);
    }
    ...
    if (evaluator.isMemberInClass(method, "org.whispersystems.libsignal.logging.Log")) {
      LintFix fix = quickFixIssueLog(call);
      context.report(LOG_NOT_APP, call, context.getLocation(call), "Using Pigeon server logger instead of app level Logger", fix);
    }
